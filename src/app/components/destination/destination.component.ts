...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, DestinationItemComponent, FormsModule],
  templateUrl: './destination.component.html',
  styleUrl: './destination.component.css',
})
export class DestinationComponent {
  initialDestinationsList: Destination[] = [
    {
      id: 1,
      name: 'Melaka Mosque',
  ...
  filterDestinations = [...this.initialDestinationsList];

  onChangeInputValue(): void {
    if (!this.searchValue) {
      this.filterDestinations = [...this.initialDestinationsList];
    }
  }

  onClickShowSearchDestinations(): void {
    this.filterDestinations = this.initialDestinationsList.filter(
      (eachDestinationName) =>
        eachDestinationName.name
          .toLowerCase()
